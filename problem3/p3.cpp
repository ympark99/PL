...
void master_mode(){
    ...
    Study study = Study(); // study 클래스 생성
    ...
    // 지점 추가
        ...
        scanf("%d", &study.uni_study); // 지점 번호 선택
        if(study.uni_study < 1 || study.uni_study > 6){
        ...
        while (!feof(fp)){	
            ...
            // 번호 겹치면 에러
            if(atoi(splitFile[1]) == study.uni_study){
        ...
        sprintf(to_str, "%d", study.uni_study);
        fputs(to_str, fp);
        fputs("|", fp);		
        fputs("\n", fp); // enter
        fclose(fp);
    ...
    // 지점 수정
        ...
        // 스터디 공간 추가
            ...
            // 지점 있는지 확인
            if(!find_file(fp, study.uni_study, 0, 1)){
            ...
            scanf("%d", &study.uni_room); // 공간 번호 선택
            if(study.uni_room < 1 || study.uni_room > 5){
            ...
            fputs("|", fp);	
            sprintf(to_str, "%d", study.uni_room);
        ...
        // 스터디 공간 수정
            ...
            // 수정할 지점의 공간 있는지 확인
            if(!find_file(fp, study.uni_study, study.uni_room, 2)){
        ...
        // 스터디 공간 삭제
            ...
            if(delete_file(fp, study.uni_study, study.uni_room, 2)) // 공간 삭제 성공 시
                fprintf(stderr, "%d 지점 %d 공간 삭제가 완료되었습니다.\n", study.uni_study, study.uni_room);
            else 
                fprintf(stdout, "%d 지점 %d 공간은 없으므로 삭제가 불가능합니다.\n", study.uni_study, study.uni_room); // 일치 번호 없는 경우
            fclose(fp);  
            return;          
        }
        else{
    ...
    // 지점 삭제
        ...
        if(delete_file(fp, study.uni_study, 0, 1)) // 지점 삭제 성공 시
            fprintf(stderr, "%d 지점 삭제가 완료되었습니다.\n", study.uni_study);
        else 
            fprintf(stdout, "%d 지점은 없으므로 삭제가 불가능합니다.\n", study.uni_study); // 일치 번호 없는 경우
        fclose(fp);
        ...
        while (!feof(fp)){
            ...
            if(!strcmp(splitFile[0], "**")) continue; // 이미 삭제 됐다면, 패스
            // 번호 겹치면 삭제
            if(atoi(splitFile[1]) == study.uni_study){ // 지점 삭제
...
void user_mode(){
    ...
    // 2. 공간 예약
        ...
        reserve_space(username, 0);
        return;
    }
    // 3. 예약 조회 및 수정
    else if(num == 3){
        // 지점 번호 확인
        FILE *fp = fopen("reserve.txt", "r+t");
        if(fp == NULL){
            fprintf(stdout, "예약된 내역이 없습니다.\n");
            return;
        }
        fprintf(stdout, "\n                  예약 정보 조회 : %-10s\n", username);
        fprintf(stdout, "------------------------------------------------------------\n");
        ...
        while (!feof(fp)){
            ...
            if(!strcmp(username, splitFile[3])){ // 사용자 id 같으면 출력
                // 예약 정보 출력
                fprintf(stdout, "%03d | 지점번호 : %d | 공간번호 : %d | %-6d %02d시 ~ %02d시 | %02d명\n", 
                ++reserve_cnt, atoi(splitFile[1]),atoi(splitFile[2]), atoi(splitFile[4]), atoi(splitFile[5]), atoi(splitFile[6]), atoi(splitFile[7]));
            }
        }
        fprintf(stdout, "------------------------------------------------------------\n");
        ...
        char restore_buf[BUF_SIZE]; // 만약 삭제 후 수정할때 에러 발생하면, 기존 data 복구
        // 기존 내역 삭제
        reserve_cnt = 1; // 카운트
        fseek(fp, 0, SEEK_SET); // 처음으로 이동
        while (!feof(fp)){
            ...
            if(!strcmp(splitFile[0], "**")) continue; // 이미 삭제 됐다면, 패스
            if(!strcmp(username, splitFile[3])){ // 사용자 id 같을때
                if(edit_cnt == reserve_cnt){ // 해당 번호라면 삭제
                    fseek(fp, ftells, SEEK_SET); // 체크 위치로 이동
                    fgets(restore_buf, BUF_SIZE, fp); // 복구 할 데이터 복사
                    fseek(fp, ftells, SEEK_SET); // 체크 위치로 이동
                    fputs("**|", fp); // **으로 삭제 표시
                    fseek(fp, ftells, SEEK_SET); // 체크 위치로 이동
                }
        ...
        fprintf(stdout, "\n       수정 정보 입력\n");
        fprintf(stdout, "----------------------------\n");
        if(reserve_space(username, 1) == -1){ // 수정한 내역 저장 실패시
            // 기존 내용 다시 저장
            ...
            fputs(restore_buf, fp);
            fprintf(stdout, "수정 실패, 기존 내역이 유지됩니다.\n");
            fclose(fp);
            return; // 에러 시 리턴
        }
        fprintf(stdout, "예약 수정이 완료되었습니다.\n");
        return;
    }
}
...
bool find_file(FILE *fp, int uni_study, int uni_room, int mode){
    ...
    while (!feof(fp)){
        ...
        if((mode == 1) && (atoi(splitFile[1]) == uni_study)) // 지점 탐색
            return true;
        else if((mode == 2) && ((atoi(splitFile[1]) == uni_study) && (atoi(splitFile[2]) == uni_room))) // 지점의 공간 탐색
            return true;
    }
    return false;
}
